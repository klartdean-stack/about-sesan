...
export default function RatingPanel({courseId, locale}: {courseId: string; locale: "km" | "en"}) {
  ...
  return <section id="rating" className="mt-10 max-w-2xl scroll-mt-8 rounded-3xl border border-white/10 bg-white/10 p-6"><p className="text-sm font-black uppercase tracking-[.18em] text-green-400">RATING</p><h2 className="mt-2 text-xl font-black">{km ? "តើមេរៀននេះមានប្រយោជន៍កម្រិតណា?" : "How useful was this lesson?"}</h2><p className="mt-2 text-sm text-slate-300">{km ? "ចុចផ្កាយពី 1 ដល់ 5។ អ្នកអាចកែ Rating វិញបាន។" : "Choose 1 to 5 stars. You can change your rating later."}</p><div className="mt-5 flex gap-2" onMouseLeave={() => setHovered(0)}>{[1,2,3,4,5].map(value => <button key={value} type="button" disabled={loading} onMouseEnter={() => setHovered(value)} onClick={() => rate(value)} aria-label={`${value} stars`} className="rounded-xl p-1 transition hover:scale-110 disabled:opacity-50"><Star className={`h-9 w-9 ${value <= active ? "fill-amber-400 text-amber-400" : "text-slate-500"}`} /></button>)}</div>{rating > 0 && <p className="mt-3 text-sm font-bold text-amber-300">{km ? `Rating របស់អ្នក៖ ${rating}/5` : `Your rating: ${rating}/5`}</p>}{message && <p className="mt-4 rounded-xl bg-white/10 p-3 text-sm font-bold text-green-300">{message}</p>}</section>;
}
