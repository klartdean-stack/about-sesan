...
export default function MyLearning({locale}: {locale: "km" | "en"}) {
  ...
  return <main className="min-h-screen bg-[#f7f9fc] px-5 py-10 text-slate-950"><div className="mx-auto max-w-6xl">
    ...
    {courses === null ? <div className="flex min-h-[40vh] items-center justify-center"><LoaderCircle className="h-12 w-12 animate-spin text-green-600" /></div>
      : loginRequired ? <div className="mx-auto mt-10 max-w-md rounded-3xl bg-white p-8 shadow-sm"><BookOpen className="mx-auto h-12 w-12 text-amber-500" /><h2 className="mt-4 text-center text-xl font-black">{km ? "ចូលគណនីដែលបានប្រើទិញ" : "Sign in to your purchasing account"}</h2><form onSubmit={login} className="mt-6 space-y-4"><input required type="email" name="email" placeholder={km ? "អ៊ីមែល" : "Email"} className="w-full rounded-2xl border border-slate-200 px-4 py-3 outline-none focus:border-green-500" /><input required type="password" name="password" placeholder={km ? "លេខសម្ងាត់" : "Password"} className="w-full rounded-2xl border border-slate-200 px-4 py-3 outline-none focus:border-green-500" />{loginError && <p className="rounded-xl bg-red-50 p-3 text-sm font-bold text-red-700">{loginError}</p>}<button disabled={loading} className="w-full rounded-2xl bg-green-600 px-5 py-3.5 font-black text-white disabled:opacity-60">{loading ? "Loading…" : km ? "ចូលគណនី" : "Sign in"}</button></form></div>
