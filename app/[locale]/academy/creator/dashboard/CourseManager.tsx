...
export default function CourseManager({session, application, locale}: {session: AcademySession; application: CreatorApplication; locale: "km" | "en"}) {
  ...
  async function submit(event: FormEvent<HTMLFormElement>) {
    ...
    const publicPreview = data.get("publicPreview") as File;
    ...
    if (publicPreview?.size > 50 * 1024 * 1024) {setMessage(t("Preview video must be under 50MB.", "វីដេអូគំរូត្រូវតូចជាង 50MB។")); setLoading(false); return;}
    try {
      ...
      const course = await submitAcademyCourse(session, {
        ...
        previewVideoUrl: uploadedPreview.publicUrl,
        previewVideoPath: uploadedPreview.path,
        previewVideoFileName: publicPreview?.size ? publicPreview.name : "",
  ...
  async function choosePublicPreview(file?: File) {
    if (draftPublicPreview) URL.revokeObjectURL(draftPublicPreview);
    if (!file?.size) {setDraftPublicPreview(""); setDraftPreviewDuration(0); return;}
    setDraftPublicPreview(URL.createObjectURL(file));
  ...
  return <div>
    ...
    {showForm && <form onSubmit={submit} className="mt-6 grid gap-4 rounded-[26px] bg-slate-50 p-5 md:grid-cols-2">
      ...
      <div className="md:col-span-2"><FileField name="publicPreview" accept="video/*" icon={Play} required={false} label={t("Free preview clip (optional, max 2 minutes / 50MB)", "វីដេអូគំរូឥតគិតថ្លៃ (មិនបង្ខំ, អតិបរមា 2 នាទី / 50MB)")} onChange={event => choosePublicPreview(event.target.files?.[0])} /></div>
