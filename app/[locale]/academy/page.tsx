...
export default async function AcademyPage({params, searchParams}: PageProps) {
  ...
  const publishedCourses = await listPublishedAcademyCourses().catch(() => []);
  const publishedFiltered = publishedCourses.filter((course) => {
    ...
    <main className="min-h-screen bg-[#f7f9fc] text-slate-950">
      ...
      <section id="courses" className="scroll-mt-24 px-5 py-20 lg:px-8">
        <div className="mx-auto max-w-7xl"><div className="flex flex-col gap-6 lg:flex-row lg:items-end lg:justify-between"><div><p className="text-sm font-black uppercase tracking-[0.2em] text-green-700">{text("Video marketplace", "ទីផ្សារមេរៀនវីដេអូ")}</p><h2 className="mt-3 text-4xl font-black tracking-tight sm:text-5xl">{text("Learn something useful today", "រៀនអ្វីដែលមានប្រយោជន៍ថ្ងៃនេះ")}</h2></div><form action={`/${locale}/academy`} className="flex w-full max-w-md overflow-hidden rounded-2xl border border-slate-200 bg-white p-2 shadow-sm"><Search className="ml-3 mt-3 h-5 w-5 text-slate-400" /><input name="q" defaultValue={filters.q} placeholder={text("Search courses...", "ស្វែងរកមេរៀន...")} className="min-w-0 flex-1 px-3 outline-none" />{category !== "all" && <input type="hidden" name="category" value={category} />}<button className="rounded-xl bg-green-600 px-5 py-3 text-sm font-black text-white">{text("Search", "ស្វែងរក")}</button></form></div>
          ...
          <div className="mt-10 grid gap-7 md:grid-cols-2 xl:grid-cols-3">{publishedFiltered.map((course) => <PublicCourseCardDetails key={course.id} course={course} locale={locale} />)}{filtered.map((course) => <CourseCard key={`sample-${course.id}`} course={course} locale={locale} />)}</div>
          {publishedFiltered.length === 0 && filtered.length === 0 && <div className="mt-10 rounded-3xl border border-slate-200 bg-white py-16 text-center"><Search className="mx-auto h-10 w-10 text-slate-300" /><p className="mt-4 text-xl font-black">{text("No courses found", "រកមិនឃើញមេរៀន")}</p><Link href={`/${locale}/academy#courses`} className="mt-5 inline-flex rounded-full bg-green-600 px-6 py-3 text-sm font-black text-white">{text("View all", "មើលទាំងអស់")}</Link></div>}
        </div>
      </section>
...
function PublishedCourseCard({course, locale}: {course: AcademyCourseRecord; locale: "km" | "en"}) {const title = locale === "km" ? course.titleKm : course.titleEn; const description = locale === "km" ? course.descriptionKm : course.descriptionEn; return <article className="group overflow-hidden rounded-[28px] border-2 border-green-200 bg-white shadow-sm transition hover:-translate-y-2 hover:shadow-xl"><div className="relative aspect-video bg-slate-900"><img src={course.coverImage} alt={title} className="h-full w-full object-cover" /><span className="absolute left-4 top-4 rounded-full bg-green-500 px-3 py-1.5 text-xs font-black text-white">NEW</span><span className="absolute inset-0 flex items-center justify-center bg-slate-950/10"><span className="flex h-14 w-14 items-center justify-center rounded-full bg-white/90 text-green-700 shadow-lg"><Play className="ml-1 h-6 w-6 fill-current" /></span></span></div><div className="p-6"><p className="text-xs font-black uppercase tracking-[0.14em] text-green-700">{categories.find(item => item.id === course.category)?.[locale] ?? course.category}</p><h3 className="mt-3 line-clamp-2 text-xl font-black leading-snug">{title}</h3><p className="mt-3 line-clamp-2 text-sm leading-6 text-slate-500">{description}</p><div className="mt-5 flex items-center justify-between border-t border-slate-100 pt-5"><p className="text-sm font-bold text-slate-500">{course.creatorName}</p><p className="text-xl font-black text-green-700">{course.priceRiel.toLocaleString()}៛</p></div><button disabled className="mt-5 inline-flex w-full cursor-not-allowed items-center justify-center gap-2 rounded-xl bg-green-600 px-5 py-3.5 text-sm font-black text-white opacity-80">{locale === "km" ? "ទិញមេរៀន — មកដល់ឆាប់ៗ" : "Buy course — Coming soon"}<ArrowRight className="h-4 w-4" /></button></div></article>;}

function PublicCourseCardDetails({course, locale}: {course: AcademyCourseRecord; locale: "km" | "en"}) {
  const km = locale === "km";
  const title = km ? course.titleKm : course.titleEn;
  const description = km ? course.descriptionKm : course.descriptionEn;
  const date = course.createdAt ? new Intl.DateTimeFormat(km ? "km-KH" : "en-GB", {day: "numeric", month: "short", year: "numeric"}).format(new Date(course.createdAt)) : "—";
  ...
  return <article className="group overflow-hidden rounded-[28px] border-2 border-green-200 bg-white shadow-sm transition hover:-translate-y-2 hover:shadow-xl">
    <div className="relative aspect-video bg-slate-900">{course.previewVideoUrl ? <video src={course.previewVideoUrl} poster={course.coverImage} controls controlsList="nodownload" preload="metadata" playsInline className="h-full w-full object-cover" /> : <img src={course.coverImage} alt={title} className="h-full w-full object-cover" />}<span className="pointer-events-none absolute left-4 top-4 rounded-full bg-green-500 px-3 py-1.5 text-xs font-black text-white">{course.previewVideoUrl ? (km ? "មើលឥតគិតថ្លៃ" : "FREE PREVIEW") : "NEW"}</span><span className="pointer-events-none absolute bottom-4 right-4 inline-flex items-center gap-1 rounded-full bg-slate-950/80 px-3 py-1.5 text-xs font-black text-white"><Clock3 className="h-3.5 w-3.5" />{course.previewVideoUrl && course.previewDurationSeconds > 0 ? `${Math.floor(course.previewDurationSeconds / 60)}:${String(course.previewDurationSeconds % 60).padStart(2, "0")}` : duration}</span></div>
