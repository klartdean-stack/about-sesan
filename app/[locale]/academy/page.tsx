...
function PublicCourseCardDetails({course, locale}: {course: AcademyCourseRecord; locale: "km" | "en"}) {
  ...
  return <article className="group overflow-hidden rounded-[28px] border-2 border-green-200 bg-white shadow-sm transition hover:-translate-y-2 hover:shadow-xl">
    <div className="relative aspect-video bg-slate-900"><img src={course.coverImage} alt={title} className="h-full w-full object-cover" /><span className="absolute left-4 top-4 rounded-full bg-green-500 px-3 py-1.5 text-xs font-black text-white">NEW</span><span className="absolute bottom-4 right-4 inline-flex items-center gap-1 rounded-full bg-slate-950/80 px-3 py-1.5 text-xs font-black text-white"><Clock3 className="h-3.5 w-3.5" />{duration}</span></div>
